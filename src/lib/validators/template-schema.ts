...
// Template categories
export const TemplateCategory = z.enum(["MARKETING", "UTILITY", "AUTHENTICATION"]);

// Template subcategories
export const TemplateSubCategory = z.enum(["ORDER_STATUS"]).optional();

// Header formats
export const HeaderFormat = z.enum(["TEXT", "IMAGE", "VIDEO", "DOCUMENT", "LOCATION"]);
...
// Component types
export const ComponentType = z.enum(["HEADER", "BODY", "FOOTER", "BUTTONS", "CAROUSEL", "LIMITED_TIME_OFFER"]);
...
const createTextValidator = (maxLength: number, allowVariables = true) => {
  return z.string()
    ...
    .refine((text) => {
      ...
      const invalidVariables = text.match(/\{\{[^}]*\}\}/g)?.filter(v => !v.match(/^\{\{\d+\}\}$/));
  ...
  buttons: z.array(ButtonSchema)
    ...
    .refine((buttons) => {
      ...
      if (quickReplyButtons.length > 0 && otherButtons.length > 0) {
        ...
        const lastQuickReplyIndex = buttons.map((b, i) => b.type === "QUICK_REPLY" ? i : -1)
          .filter(i => i !== -1).pop() || -1;
    ...
    .refine((buttons) => {
      ...
      return phoneButtons.length <= 1 && urlButtons.length <= 2 && copyCodeButtons.length <= 1;
...
export const WhatsAppTemplateSchema = z.object({
  ...
  language: z.string()
    ...
    .regex(/^[a-z]{2}(_[A-Z]{2})?$/, "Invalid language code format (e.g., en_US, es, fr)"),
  category: TemplateCategory,
  subCategory: TemplateSubCategory,
  messageSendTtlSeconds: z.number()
    .min(30)
    .max(2592000)
    .optional(),
  components: z.array(ComponentSchema)
    ...
    .refine((components) => {
      ...
      return headerComponents.length <= 1 && footerComponents.length <= 1 && buttonComponents.length <= 1;
